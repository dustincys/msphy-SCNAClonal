...
class Datum(object):
    def __init__(self, name, id, a, d, mu_r=0, mu_v=0):
        ...
        self.mu_v = mu_v
        self._log_bin_norm_const = [u.log_bin_coeff(
            self.d[tp], self.a[tp]) for tp in arange(len(self.a))]
        ...
        # this is just a pointer to tssb (tree object), gets initialized in
        # evolve.py
        self.tssb = None
    ...
    def _log_likelihood(self, phi, update_tree=True, new_state=0):
        ...
        return sum(
            [self.__log_likelihood__(phi[tp],
                                     tp, update_tree, new_state)
             for tp in arange(ntps)])

    # new_state is set to 0 or 1 during Metropolis-Hastings updates, defaults
    # to 0 in all other places
    ...
    def __log_complete_likelihood1__(self, phi, mu_r, mu_v, new_state=0):
        ...
        if self.cnv:
            ...
            mu = (1 - phi) * mu_r + phi*mu_v  # (mu_r=0.999, mu_v=0.5)
            llh = u.log_binomial_likelihood(
                self.a, self.d, mu) + self._log_bin_norm_const
        return llh
    ...
    def compute_n_genomes1(self, maternal, new_state=0):
        ...
        for node in ancestors:
            # this is needed for Metropolis-Hastings likelihood computations
            pi = node.pi1 if new_state else node.pi
        ...
        # do this after the SSM node, i.e, for all nodes in the subtree below
        # the SSM node
        def descend(nd):
            # this is needed for Metropolis-Hastings likelihood computations
            pi = nd.pi1 if new_state else nd.pi
